    ...
    namespace {
        ...
        bool upload_all_to_current_context() {
            ...
            for (std::size_t index = 0; index < kEntries.size(); ++index) {
                ...
                LOG_INFO("Water texture uploaded: {} {}x{} (unit {})", kEntries[index].file, g_images[index].width,
                         g_images[index].height, kEntries[index].unit);
            }
            g_uploaded = true;
            return true;
        }
    ...
    bool ensure_water_textures_bound() noexcept {
        try {
            ...
            if (!g_uploaded || context != g_ownerContext ||
                std::any_of(g_textures.begin(), g_textures.end(), [](unsigned texture) { return texture == 0; })) {
