...
mediump float ieeV1FpspriteMarker()
{
    return 0.0;
}
...
lowp vec4 uhBlurBackdrop(in mediump vec2 tcStep)
{
    const int mSize = 5;
    ...
    for (int i = -kSize; i <= kSize; ++i)
    {
        ...
        {
            lowp vec4 sample = uhSampleSprite(vTc + vec2(float(i), float(j)) * tcStep);
            blurColour += kernel[kSize + j] * kernel[kSize + i] * sample;
            if (sample.a > 0.5)
            {
...
void main()
{
    mediump vec2 tcStep = uhSpriteTcScale();
    lowp vec4 baseTex = texture2D(uTex, vTc);
    lowp vec3 reconstructionRgb = baseTex.rgb;
    if (uhHasTcScale())
    {
        lowp float reconstructionMix = smoothstep(0.45, 0.95, baseTex.a);
        reconstructionMix = clamp(reconstructionMix * 0.35 + ieeV1FpspriteMarker(), 0.0, 1.0);
        reconstructionRgb = mix(baseTex.rgb, uhSampleEasu(vTc), reconstructionMix);
    }
    lowp vec3 enhancedRgb = uhSharpenSprite(vTc, reconstructionRgb, baseTex.a);
    lowp vec4 texColor = vec4(enhancedRgb, baseTex.a);

    if (uSpriteBlurAmount <= 0.001)
    {
        gl_FragColor = texColor;
        return;
    }

    lowp vec4 blurColour = uhBlurBackdrop(tcStep);
    gl_FragColor = mix(blurColour, texColor, texColor.a);
}
